...
async function getWeatherDataForLocation() {
    ...
    const [currentConditions, upcomingWeather] = await Promise.all([service.getCurrentConditions(location.code), service.getUpcomingWeather(location.code)]);
    ...
    document.querySelector("#upcoming").appendChild(createUpcomingWeatherContainer(upcomingWeather));
}
...
function createUpcomingWeatherContainer(forecast) {
    ...
    forecast.forecast.forEach((data) => {
        ...
        element.appendChild(createForecastElement(weatherSymbols[data.condition]), "symbol");
...
function createCurrentWeatherContainer(forecast) {
    ...
    currentWeatherContainer.appendChild(createForecastElement(weatherSymbols[forecast.forecast.condition], "condition", "symbol"));
...
function createForecastDataContainer(forecast) {
    ...
    container.appendChild(createForecastElement(`${forecast.forecast.low}°/${forecast.forecast.high}°`, "forecast-data"));
    container.appendChild(createForecastElement(weatherSymbols[forecast.forecast.condition], "forecast-data"));
